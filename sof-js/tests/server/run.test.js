...
describe('$viewdefinition-run operation', () => {
  test('returns JSON results for a ViewDefinition', async () => {
    const response = await fetch(
      'http://localhost:3002/ViewDefinition/patient_demographics/$viewdefinition-run?format=json',
    )
  ...
  test('CSV output includes header row by default', async () => {
    const response = await fetch(
      'http://localhost:3002/ViewDefinition/patient_demographics/$viewdefinition-run?format=csv',
    )
  ...
  test('CSV output excludes header row when header=false', async () => {
    const response = await fetch(
      'http://localhost:3002/ViewDefinition/patient_demographics/$viewdefinition-run?format=csv&header=false',
    )
  ...
  test('CSV output includes header row when header=true', async () => {
    const response = await fetch(
      'http://localhost:3002/ViewDefinition/patient_demographics/$viewdefinition-run?format=csv&header=true',
    )
