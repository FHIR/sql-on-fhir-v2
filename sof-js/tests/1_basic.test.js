...
let resources = [
  { id: 'pt1', name: [{ family: 'F1' }], active: true },
  { id: 'pt2', name: [{ family: 'F2' }], active: false },
  { id: 'pt3' },
]
...
describe('basics', () => {
  ...
  add_test({
    title: 'where',
    view: { select: [{ column: [{ name: 'id', path: 'id' }] }], where: [{ path: 'active=true' }] },
    expect: [{ id: 'pt1' }],
  })

  add_test({
    title: 'where',
    view: { select: [{ column: [{ name: 'id', path: 'id' }] }], where: [{ path: 'active=false' }] },
    expect: [{ id: 'pt2' }],
  })

  add_test({
    title: 'where as element',
    view: { select: [{ column: [{ name: 'id', path: 'id' }] }], where: [{ path: 'active' }] },
    expect: [{ id: 'pt1' }],
  })

  add_test({
    title: 'where as expr - 1',
    view: { select: [{ column: [{ name: 'id', path: 'id' }] }], where: [{ path: "name.family = 'F2'" }] },
    expect: [{ id: 'pt2' }],
  })

  add_test({
    title: 'where as expr - 2',
    view: { select: [{ column: [{ name: 'id', path: 'id' }] }], where: [{ path: "name.family = 'F1'" }] },
    expect: [{ id: 'pt1' }],
  })

  add_test({
    title: 'where as name.family',
    view: { select: [{ column: [{ name: 'id', path: 'id' }] }], where: [{ path: 'name.family' }] },
