# sqlquery-run-server Specification

## Purpose

The `$sqlquery-run` operation runs SQLQuery Library resources - SQL packaged as
a Library together with its ViewDefinition dependencies - against a server's
data, returning results in a caller-selected format. This spec describes how
the reference server resolves Libraries (by reference, inline resource, or
instance URL), materialises ViewDefinition dependencies into temporary tables,
binds named input parameters, and shapes responses and errors.
...
- **THEN** the server returns HTTP 422 with an OperationOutcome indicating `processing`
