...
Specification contains set of tests in `/tests` directory.
Tests are set of test case files, each case covers one aspect of implementation.
Test case is represented as JSON document. It has title and description attributes, 
set of fixtures (FHIR resources) as `resources` attribute, and array of test objects.

Test object has unique `title`, ViewDefinition at `view` attribute, and expected set of resulting
rows in `expect` attribute.
...
- **Title**: A brief, descriptive title that summarizes the aspect of the
  implementation being tested.
- **Description**: A detailed explanation of what the test case aims to
...
- **Test Objects**: An array of objects, each representing a unique test
  scenario within the case. Every test object includes:
  - **Title**: A unique, descriptive title for the test object, differentiating
...
as a git submodule to your project. Furthermore, implementers are advised to
...
outcomes of your implementation's test runs, the next step is to make these
...
your implementation's compliance with the specifications. This guide outlines
...
available but also contribute to a collective resource that benefits the entire
